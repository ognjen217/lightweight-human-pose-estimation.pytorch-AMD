...
__global__ void resize_cubic_profile_kernel(
...
__global__ void vertical_max_profile_kernel(
...
__global__ void horizontal_max_profile_kernel(
...
__global__ void topk_from_pooled_profile_kernel(
    const float* __restrict__ resized,
...
extern "C" int heatmap_topk_hip_run_host_profile(
    ...
    if (!heatmaps_host || !top_scores_host || !top_indices_host || !profile) {
...

    {
        ...
        hipLaunchKernelGGL(
            topk_from_pooled_profile_kernel,
            dim3(total_bc),
            dim3(threads),
            shared_bytes,
            stream,
    ...
    if (err != hipSuccess) { status = HIP_TOPK_HIP_ERROR; goto cleanup; }
    (void)hipEventRecord(events[7], stream);

    err = hipEventSynchronize(events[7]);
    if (err != hipSuccess) { status = HIP_TOPK_HIP_ERROR; goto cleanup; }
    ...
    profile->horizontal_ms = elapsed_ms(events[3], events[4]);
    profile->topk_ms = elapsed_ms(events[4], events[5]);
    profile->d2h_scores_ms = elapsed_ms(events[5], events[6]);
    profile->d2h_indices_ms = elapsed_ms(events[6], events[7]);
    profile->device_total_ms = elapsed_ms(events[1], events[5]);
    profile->total_ms = elapsed_ms(events[0], events[7]);
