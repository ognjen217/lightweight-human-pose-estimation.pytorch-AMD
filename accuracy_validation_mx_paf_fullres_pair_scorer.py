...
COCO AP/AR validation for:
  ...
  mx_cubic_topk_fullres_paf_scorer_k20
    Experimental path:
      - manual cubic heatmap resize + NMS + TopK in MIGraphX
      - full-res cubic PAF sampling + limb pair scoring in MIGraphX
      - CPU connection NMS + pose assembly only

This is the accuracy counterpart of:
  speed_validation_mx_paf_fullres_pair_scorer.py

Expected experimental modules:
  modules/migraphx_manual_cubic_topk_compiler.py
  modules/migraphx_paf_fullres_pair_scorer.py
  modules/migraphx_paf_fullres_pair_scorer_compiler.py
...
from modules.migraphx_paf_fullres_pair_scorer import MIGraphXPAFFullResPairScorer
from modules.migraphx_paf_fullres_pair_scorer_compiler import (
    compile_paf_fullres_pair_scorer_head,
    head_name as paf_fullres_scorer_head_name,
)
...
class MXRRunner:
    """Small wrapper for the manual-cubic heatmap TopK MXR head."""
...
class VariantMeter:
    ...
    paf_fullres_scorer_mx_ms: List[float] = field(default_factory=list)
    ...
    def add_timing(self, pre: float, infer: float, decode: float, post: float, timings: Mapping[str, Any]) -> None:
        ...
        self.paf_fullres_scorer_mx_ms.append(float(timings.get("paf_fullres_scorer_mx", 0.0) or 0.0))
    ...
    def latency_summary(self) -> Dict[str, float]:
        ...
        return {
            ...
            "paf_fullres_scorer_mx_ms": mean(self.paf_fullres_scorer_mx_ms),
...
_PAF_FULLRES_SCORER_CACHE: Dict[str, MIGraphXPAFFullResPairScorer] = {}
...
def resolve_paf_fullres_scorer_path(
    *,
    ...
    name = paf_fullres_scorer_head_name(
        paf_h,
        paf_w,
        full_h,
        full_w,
        ...
        cubic_a=args.paf_cubic_a,
    )
    path = Path(args.paf_fullres_cache_dir) / f"{name}.mxr"
    if path.exists():
        return path

    if not args.compile_heads:
        raise FileNotFoundError(
            f"Missing full-res PAF pair scorer head: {path}\n"
            "Run with --compile-heads or precompile required heads."
        )
    ...
    return compile_paf_fullres_pair_scorer_head(
        ...
        full_h=full_h,
        full_w=full_w,
        output_dir=args.paf_fullres_cache_dir,
        topk=args.topk,
        points_per_limb=args.points_per_limb,
        min_paf_score=args.min_paf_score,
        success_ratio_thr=args.success_ratio_thr,
...
def get_paf_fullres_scorer(path: Path) -> MIGraphXPAFFullResPairScorer:
    key = str(path)
    if key not in _PAF_FULLRES_SCORER_CACHE:
        _PAF_FULLRES_SCORER_CACHE[key] = MIGraphXPAFFullResPairScorer(key)
    return _PAF_FULLRES_SCORER_CACHE[key]


def postprocess_mx_fullres_paf_scorer(
    heatmaps: np.ndarray,
    pafs: np.ndarray,
    original_hw: Tuple[int, int],
    args,
    ...
    paf_path = resolve_paf_fullres_scorer_path(
        paf_hw=pafs.shape[:2],
        original_hw=original_hw,
        args=args,
    )
    ...
    paf_scorer = get_paf_fullres_scorer(paf_path)
    ...
    timings["paf_fullres_scorer_mx"] = t.ms
...
def write_flat_csv(path: str | Path, rows: List[Dict[str, Any]]) -> None:
    ...
    preferred = [
        ...
        "manual_cubic_mx_topk_ms", "paf_fullres_scorer_mx_ms",
...
def print_summary_table(rows: List[Dict[str, Any]]) -> None:
    print("\n" + "=" * 198)
    print("MX CUBIC TOPK + MX FULLRES PAF SCORER ACCURACY SUMMARY")
    print("=" * 198)
    print(
        f"{'variant':<46} {'img':>5} "
        f"{'AP':>6} {'AP50':>6} {'AP75':>6} {'AR':>6} "
        f"{'topk':>8} {'paf_full':>9} {'adapt':>8} {'asm':>8} "
        f"{'post':>8} {'e2e':>8} {'fps':>8} {'speedup':>8}"
    )
    print("-" * 198)
    for r in rows:
        print(
            f"{r['variant']:<46} {int(r['images']):>5} "
            ...
            f"{float(r.get('paf_fullres_scorer_mx_ms', 0.0)):>9.2f} "
...
def run(args) -> List[Dict[str, Any]]:
    ...
    ensure_dir(args.paf_fullres_cache_dir)
    ...
    mx_name = f"mx_cubic_topk_fullres_paf_scorer_k{args.topk}"
    meters = {
    ...
    print("\nRunning COCO accuracy validation for MX cubic TopK + MX full-res PAF scorer")
    ...
    print(f"Heatmap cubic a:   {args.cubic_a}")
    print(f"PAF cubic a:       {args.paf_cubic_a}")
    ...
    print(f"PAF full cache:    {args.paf_fullres_cache_dir}")
    ...
    for i, sample in enumerate(dataset):
        ...
        # Experimental full-res cubic PAF scorer path
        poses, kpts, timings = postprocess_mx_fullres_paf_scorer(heatmaps, pafs, original_hw, args)
    ...
    summary_json = Path(args.output_dir) / "accuracy_mx_paf_fullres_pair_scorer_summary.json"
    summary_csv = Path(args.output_dir) / "accuracy_mx_paf_fullres_pair_scorer_summary.csv"
...
def parse_args():
    p = argparse.ArgumentParser(description="COCO AP/AR validation for MX cubic TopK + MX full-res PAF scorer.")
    ...
    p.add_argument("--output-dir", default="outputs/accuracy_mx_paf_fullres_pair_scorer")
    ...
    p.add_argument("--cubic-a", type=float, default=-0.75, help="Manual cubic parameter used by heatmap TopK head.")
    p.add_argument("--paf-cubic-a", type=float, default=-0.75, help="Manual cubic parameter used by full-res PAF scorer.")
    ...
    p.add_argument("--paf-fullres-cache-dir", default="models/paf_fullres_pair_scorer_coco_cache")
