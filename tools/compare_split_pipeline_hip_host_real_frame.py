#!/usr/bin/env python3
"""Compare split MXR + HIP heatmap backend on real video frames.

The random-input compare is useful for deterministic graph-contract testing, but
it often produces no valid heatmap peaks above threshold.  This tool builds the
B-sized input tensor from real frames using the same video preprocessing used by
speed_validation.py:

    cv2.resize(frame, (968, 544))
    (frame.astype(float32) - 128) / 256
    HWC -> CHW
...
Then it compares:
    ...
    merged MXR
vs
    MXR1 -> native HIP host-mediated heatmap TopK -> MXR2
"""
...
def preprocess_frame(frame: np.ndarray, *, target_w: int, target_h: int, dtype: str) -> np.ndarray:
    img = cv2.resize(frame, (int(target_w), int(target_h)))
...
def read_video_batch(
    ...
    *,
    ...
    for b in range(int(batch_size)):
        idx = int(frame_index) + b * int(batch_stride)
        if frame_count > 0:
            idx = idx % frame_count
        ...
        if not ok or frame is None:
            ...
            reused = False
        frames.append(preprocess_frame(frame, target_w=target_w, target_h=target_h, dtype=dtype))
        meta.append({"batch_index": b, "frame_index": idx, "reused_previous": bool(reused)})
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Real-frame split-vs-merged comparison using HIP host heatmap backend.")
    ...
    p.add_argument("--run-frame-stride", type=int, default=24, help="Frame offset added between runs.")
    p.add_argument("--batch-frame-stride", type=int, default=1, help="Frame offset between samples inside the batch.")
    ...
    p.add_argument("--require-valid-topk", action="store_true", help="Fail if every run has valid_topk_count == 0.")
    p.add_argument("--json", default="outputs/split_pipeline_compare/b4_hip_host_real_frame.json")
    p.add_argument("--markdown", default="outputs/split_pipeline_compare/b4_hip_host_real_frame.md")
    return p.parse_args()
