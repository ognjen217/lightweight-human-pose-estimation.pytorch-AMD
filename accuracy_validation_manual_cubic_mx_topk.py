#!/usr/bin/env python3
"""
COCO AP/AR validation for manual cubic resize + MIGraphX NMS + TopK postprocess.

Compares:
  1) optimized_batch_k20_fast
     Existing CPU reference-like optimized path:
       CPU cv2.INTER_CUBIC heatmap/PAF resize
       CPU batched K extraction
       CPU group_keypoints_fast

  2) migraphx_manual_cubic_nms_topk_k{K}
     Experimental graph path:
       low-res heatmaps -> manual cubic resize + NMS + TopK in MIGraphX
       CPU cv2.INTER_CUBIC PAF resize
       CPU group_keypoints_fast

Notes:
  - This script intentionally avoids compiling heads inside the main inference
    loop. It can compile on demand per new shape, but that is disabled by
    default. For large COCO runs, precompile heads first or use --compile-heads
    knowing it may take time.
  - Manual cubic head avoids ONNX Resize(mode="cubic") entirely.
"""
...
class VariantMeter:
    ...
    def add_timing(self, pre: float, infer: float, decode: float, post: float, timings: Dict[str, Any]) -> None:
        ...
        self.paf_resize_ms.append(float(timings.get("resize_pafs", timings.get("paf_resize_ms", 0.0)) or 0.0))
        self.topk_adapter_ms.append(float(timings.get("topk_adapter", 0.0) or 0.0))
        self.group_ms.append(float(timings.get("group_keypoints", timings.get("group_total", 0.0)) or 0.0))
...
_HEAD_CACHE: Dict[str, MIGraphXResizeNMSTopKHead] = {}


def resolve_manual_cubic_head_path(
    *,
    ...
    path = Path(args.head_cache_dir) / f"{name}.mxr"
    ...
    if not args.compile_heads:
        raise FileNotFoundError(
            f"Missing manual cubic head: {path}\n"
            "Run with --compile-heads or precompile the required heads first."
        )
    ...
    return compile_manual_cubic_nms_topk_head(
        ...
        out_h=out_h,
        out_w=out_w,
        output_dir=args.head_cache_dir,
        channels=18,
        topk=args.topk,
        threshold=args.threshold,
        nms_radius=args.nms_radius,
...
def get_head(path: Path) -> MIGraphXResizeNMSTopKHead:
    key = str(path)
    if key not in _HEAD_CACHE:
        _HEAD_CACHE[key] = MIGraphXResizeNMSTopKHead(key)
    return _HEAD_CACHE[key]


def postprocess_manual_cubic_topk(
    heatmaps: np.ndarray,
    pafs: np.ndarray,
    original_hw: Tuple[int, int],
    args,
    ...
    head_path = resolve_manual_cubic_head_path(
        heatmaps_hw=heatmaps.shape[:2],
        original_hw=original_hw,
        args=args,
    )
    head = get_head(head_path)
    ...
    with Timer() as t:
        try:
            out = group_keypoints_fast(all_kpts, pafs_full, points_per_limb=args.points_per_limb, return_timing=True)
        except TypeError:
            out = group_keypoints_fast(all_kpts, pafs_full, points_per_limb=args.points_per_limb)
...
def write_flat_csv(path: str | Path, rows: List[Dict[str, Any]]) -> None:
    ...
    preferred = [
        ...
        "manual_cubic_mx_topk_ms", "paf_resize_ms", "topk_adapter_ms", "group_ms",
...
def print_summary_table(rows: List[Dict[str, Any]]) -> None:
    print("\n" + "=" * 180)
    print("MANUAL CUBIC + MX NMS TOPK ACCURACY SUMMARY")
    print("=" * 180)
    print(
        f"{'variant':<42} {'img':>5} "
        f"{'AP':>6} {'AP50':>6} {'AP75':>6} {'AR':>6} "
        f"{'manual_topk':>12} {'paf':>8} {'adapt':>8} {'group':>8} "
    ...
    for r in rows:
        print(
            ...
            f"{float(r.get('manual_cubic_mx_topk_ms', 0.0)):>12.2f} "
            f"{float(r.get('paf_resize_ms', 0.0)):>8.2f} "
            f"{float(r.get('topk_adapter_ms', 0.0)):>8.2f} "
            f"{float(r.get('group_ms', 0.0)):>8.2f} "
...
def run(args) -> List[Dict[str, Any]]:
    ...
    ensure_dir(args.head_cache_dir)
    ...
    manual_name = f"migraphx_manual_cubic_nms_topk_k{args.topk}"
    meters = {
        baseline_name: VariantMeter(model=model_name, variant=baseline_name),
        manual_name: VariantMeter(model=model_name, variant=manual_name),
    }
    ...
    print("\nRunning COCO accuracy validation for manual cubic + MIGraphX NMS+TopK")
    ...
    for i, sample in enumerate(dataset):
        ...
        # Manual cubic MIGraphX path
        poses, kpts, timings = postprocess_manual_cubic_topk(heatmaps, pafs, original_hw, args)
        meters[manual_name].add_timing(
            pre=infer_timings["preprocess_ms"],
            infer=infer_timings["inference_ms"],
            decode=infer_timings["decode_ms"],
        ...
        meters[manual_name].detections.extend(build_coco_detections(image_id, poses, kpts))
    ...
    summary_json = Path(args.output_dir) / "accuracy_manual_cubic_mx_topk_summary.json"
    summary_csv = Path(args.output_dir) / "accuracy_manual_cubic_mx_topk_summary.csv"
...
def parse_args():
    p = argparse.ArgumentParser(description="COCO AP/AR validation for manual cubic resize + MIGraphX NMS+TopK.")
    ...
    p.add_argument("--output-dir", default="outputs/accuracy_manual_cubic_mx_topk")
    ...
    p.add_argument("--head-cache-dir", default="models/manual_cubic_nms_topk_coco_cache")
